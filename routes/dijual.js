...
router.get("/", requireUserOrAdmin, (req, res) => {
  ...
  if (role === "admin") {
    db.query(
      "SELECT d.*, a.AsetId, a.NamaAset FROM dijual d LEFT JOIN aset a ON d.aset_id = a.id",
      (err, rows) => {
  ...
  const q = `SELECT d.*, a.AsetId, a.NamaAset FROM dijual d LEFT JOIN aset a ON d.aset_id = a.id WHERE ${clause}`;
...
router.get("/aset/:asetId", requireUserOrAdmin, (req, res) => {
  ...
  if (role === "admin") {
    db.query(
      "SELECT d.*, a.AsetId, a.NamaAset FROM dijual d LEFT JOIN aset a ON d.aset_id = a.id WHERE a.AsetId = ?",
      [asetId],
      (err, rows) => {
        if (err) return res.status(500).json(err);
  ...
  if (!beban || beban.length === 0)
    return res
      ...
      .json({ message: "Akses ditolak: beban tidak ditemukan" });
  const { clause, params } = buildBebanFilterSQL("a.Beban", beban);
  const q = `SELECT d.*, a.AsetId, a.NamaAset FROM dijual d LEFT JOIN aset a ON d.aset_id = a.id WHERE ${clause} AND a.AsetId = ?`;
  params.push(asetId);
  db.query(q, params, (err, rows) => {
    if (err) return res.status(500).json(err);
    res.json(rows.map(mapRow));
...
router.post("/", requireUserOrAdmin, (req, res) => {
  ...
  db.query(checkAsetQ, [data.AsetId], (err, rows) => {
    ...
    const insertDijual = (uid) => {
      const q = `INSERT INTO dijual (aset_id, TglDijual, HargaJual, Pembeli, jumlah_dijual, catatan) VALUES (?, ?, ?, ?, ?, ?)`;
      const vals = [
        asetDbId,
        data.tanggal_jual,
      ...
      db.query(q, vals, (err2, result) => {
        ...
        // Decrease aset jumlah and update StatusAset to 'dijual'
        db.query(
          "UPDATE aset SET jumlah = GREATEST(jumlah - ?, 0), StatusAset = 'dijual' WHERE AsetId = ?",
          [jumlahDijual, data.AsetId],
          (errUpdate) => {
            if (errUpdate) {
              console.error("[dijual] Error updating aset:", errUpdate);
            } else {
              console.log(
                `[dijual] Decreased jumlah and set StatusAset='dijual' for AsetId=${data.AsetId}`
              );
            }
          }
        );
...
router.put("/:id", requireAdmin, (req, res) => {
  ...
  db.query("SELECT * FROM dijual WHERE id = ?", [id], (err, rows) => {
    if (err) return res.status(500).json(err);
    if (!rows || rows.length === 0)
      return res
        .status(404)
    ...
    if (updates.length === 0) {
      return res.json({ message: "Tidak ada perubahan", dijual: mapRow(old) });
    }

    values.push(id);
    const q = `UPDATE dijual SET ${updates.join(", ")} WHERE id = ?`;
    db.query(q, values, (err2) => {
      ...
      db.query(
        ...
        (err3, rows3) => {
          ...
          res.json({
            message: "Data penjualan diperbarui",
            dijual: mapRow(rows3[0]),
          });
        }
      );
...
router.delete("/:id", requireAdmin, (req, res) => {
  ...
  db.query("SELECT * FROM dijual WHERE id = ?", [id], (err, rows) => {
    ...
    db.query("DELETE FROM dijual WHERE id = ?", [id], (err2) => {
      if (err2) return res.status(500).json(err2);
      ...
      res.json({ message: "Data penjualan dihapus" });
    });
  });
});
