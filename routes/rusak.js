...
router.get("/", requireUserOrAdmin, (req, res) => {
  ...
  const q = `SELECT r.*, a.AsetId, a.NamaAset FROM rusak r LEFT JOIN aset a ON r.aset_id = a.id WHERE ${clause}`;
...
router.get("/aset/:asetId", requireUserOrAdmin, (req, res) => {
  ...
  if (!beban || beban.length === 0)
    return res
      ...
      .json({ message: "Akses ditolak: beban tidak ditemukan" });
  const { clause, params } = buildBebanFilterSQL("a.Beban", beban);
  const q = `SELECT r.*, a.AsetId, a.NamaAset FROM rusak r LEFT JOIN aset a ON r.aset_id = a.id WHERE ${clause} AND a.AsetId = ?`;
  params.push(asetId);
  db.query(q, params, (err, rows) => {
    if (err) return res.status(500).json(err);
    res.json(rows.map(mapRow));
...
router.post("/", requireUserOrAdmin, (req, res) => {
  ...
  db.query(checkAsetQ, [data.AsetId], (err, rows) => {
    ...
    const insertRusak = (uid) => {
      const q = `INSERT INTO rusak (aset_id, TglRusak, Kerusakan, jumlah_rusak, StatusRusak, catatan) VALUES (?, ?, ?, ?, ?, ?)`;
      const vals = [
        asetDbId,
        data.TglRusak,
        data.Kerusakan ?? null,
      ...
      db.query(q, vals, (err2, result) => {
        ...
        // Decrease aset jumlah and update StatusAset to 'rusak'
        db.query(
          "UPDATE aset SET jumlah = GREATEST(jumlah - ?, 0), StatusAset = 'rusak' WHERE AsetId = ?",
          [jumlahRusak, data.AsetId],
          (errUpdate) => {
            if (errUpdate) {
              console.error("[rusak] Error updating aset:", errUpdate);
            } else {
              console.log(
                `[rusak] Decreased jumlah by ${jumlahRusak} and set StatusAset='rusak' for AsetId=${data.AsetId}`
              );
            }
          }
        );
